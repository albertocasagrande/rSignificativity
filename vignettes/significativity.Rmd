...
Statistical coefficients, such as Cohen's $\kappa$ [@Cohen:1960] and
$\textrm{IA}$ [@Casagrande:2020b],
measure the agreement between two discrete
...
# rSignificativity implements some statistical coefficients
...
The statistical coefficient values alone lack of a
...
the probability of choosing by chance a matrix whose $\sigma$-value smaller than or equal to $c$.
...
$$
where $\Delta^{(n^2-1)}$ is $n^2$-dimensional probability simplex and $V(\cdot)$ is the $n^2-1$-dimensional Lebesgue measure [@significativity].

If $\sigma(M) \leq c$ is definable in an o-minimal theory [@Dries_1998], such as in the case
